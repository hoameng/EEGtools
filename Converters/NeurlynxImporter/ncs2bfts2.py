...
def ncs2bfts(startDateTime, dirName, fileNum, bfFile):
    ...
    for i in fileNum:
        ...
        ncs = ncs2py.load_ncs(fileName) # import neuralynx data. NOTE: import stores information as a dictionary

        sampleRate = ncs['sampling_rate'] 
        rawData = ncs['data']
        rawDataNew = sg.decimate(rawData,10) # downsample to 3kHz by reducing the orignal 30kHz by a factor of 10
